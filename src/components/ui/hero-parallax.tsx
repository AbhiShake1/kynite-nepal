  ...
  return (
    <div className="max-w-7xl relative mx-auto py-20 md:py-40 px-4 w-full left-0 top-0">
      <motion.h1
        ...
        className="text-4xl md:text-7xl font-bold bg-gradient-to-r from-blue-600 to-cyan-500 bg-clip-text text-transparent dark:from-blue-400 dark:to-cyan-300"
      >
      ...
      <motion.p
        ...
        className="max-w-2xl text-base md:text-xl mt-8 text-gray-700 dark:text-gray-300"
      >
...
export const ProductCard = ({
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      key={product.title}
      className="group/product h-96 w-[30rem] relative shrink-0 rounded-3xl overflow-hidden shadow-2xl"
    >
      <a
        href={product.link}
      ...
      <div className="absolute inset-0 h-full w-full bg-gradient-to-t from-black/80 via-transparent to-transparent rounded-3xl opacity-0 group-hover/product:opacity-100 transition-opacity duration-300 flex items-end justify-start p-6"></div>
      <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black/70 to-transparent rounded-3xl opacity-0 group-hover/product:opacity-100 transition-opacity duration-300">
        <h2 className="text-white text-xl font-semibold mb-2">{product.title}</h2>
        <div className="flex items-center">
          <span className="text-cyan-300 text-sm">View details</span>
          <svg
